...
## Give me that paper already

Come on, search for the command _New Folder from Git_, just as I told you in the previous section. You can copy URLs of GitHub repositories directly. I created an example for you. It's at <https://github.com/lbm364dl/test-paper>. You can pay a visit if you're interested or just copy and paste the URL:

![First step in your future relationship with Quarto](imgs/new_folder_from_git.png)

And we get our example Quarto project! Open the `index.qmd` file:

![Welcome Alice Smith from University of Example](imgs/open_index_qmd.png)
...
If you want to know more about renv, go visit the [Virtual environments with renv](renv.qmd) chapter or read any better guide, even the [official renv docs](https://rstudio.github.io/renv/articles/renv.html). If for some reason you're an renv hater, just remove `source("renv/activate.R")` from the `.Rprofile` file and then you can also get rid of the `renv` folder and the `renv.lock` file. But if you're really an renv hater, you probably already knew that.

## ...tiny tiny paper

OK, OK, we installed some packages, we should get no errors now. Just hit again that _Preview_ button. 

If it failed again, I hope it's because you're missing a LaTeX installation. Then you should just open the _terminal_ tab and install using the command `quarto install tinytex`. TinyTex, as the same suggests, is just a tiny version of LaTeX with only the most basic things we need (a full LaTeX installation can take ages).
