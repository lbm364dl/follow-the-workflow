...
2. Open [this link](https://console.cloud.google.com/auth/overview/create) to create an _app_ (whatever that means). You might be asked to create a project first. Just do that. Then try again. Fill things:
    - App Information:
...
8. Restart your R session. Your credentials are read automatically from `.Rprofile` every time you open an R session here. If you already had an open session while setting the credentials file, it won't be read until you restart the session. From Positron, you can either close and open it again, or run the command _Interpreter: Restart Activate Interpreter Session_.
9. Actually use trackdown! Check the [previous section](#sync) for the important functions. When using trackdown update/download functions, the first time you will be asked to grant access in your browser. **Toggle the _allow read/write access_ button before continuing**. Otherwise it won't work. If you forgot to do this, repeat from step 6 and you should be asked again.
